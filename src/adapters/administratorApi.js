...
export const administratorApi = {
  ...
  async getUsers(query) {
    ...
    const result = await request(`/api/administrator/users${queryString}`)
    return Array.isArray(result)
      ? result.map(mapUser)
      : { ...result, content: (result?.content || []).map(mapUser) }
  },
  ...
  async setRoleStatus(id, status) {
    return mapRole(
      await request(
        `/api/administrator/roles/${encodeURIComponent(id)}/status?status=${encodeURIComponent(status)}`,
        { method: 'PATCH' },
      ),
    )
  },
  ...
  async saveUserRoleAssignments(id, assignments) {
    return mapUser(
      await request(`/api/administrator/users/${encodeURIComponent(id)}/role-assignments`, {
        method: 'PUT',
        body: assignments,
      }),
    )
